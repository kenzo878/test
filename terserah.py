...
while True:
    ...
    try:
        with open(filename, "r", encoding='utf-8') as file:
            ...
            # Ensure that 'note' has at least 3 elements before accessing 'note[2]'
            if len(note) >= 3:
                tags = note[2].split(' ')  # Split tags if they exist
                note[2] = tags
            else:
                note.append([])  # Add an empty tag list if the file doesn't have enough lines
